var d = function( sketch ) {
  ...
  sketch.setup = function() {
    ...
    myCanvas.parent('divergenceContainer');
  ...
  sketch.draw = function() {
    ...
    for (var i = locs.length - 1; i >= 0; i--) {
      ...
      var x = locs[i].x;
      var y = locs[i].y;
      var deltaX = (sketch.width/2) - x;
      var deltaY = (sketch.height/2) - y;
    ...
    sketch.noStroke();
    sketch.drawGradient(sketch.width/2, sketch.height/2);
  ...
  sketch.drawGradient = function(x, y) {
    ...
    for (var r = radius; r > 0; --r) {
      ...
      var col = sketch.map(slider.value, -50, 50, 0, 255);
      sketch.fill(255-col, 0, col, alpha);
  ...
  document.getElementById('divergenceContainer').onmouseover = function() {
    sketch.loop(); // start looping
  }
  document.getElementById('divergenceContainer').onmouseout = function() {
    sketch.noLoop(); // stop looping
  }

  $(window).resize(function(event) {
    document.getElementById('divergenceContainer').style.height = sketch.windowWidth/3;
...
var myD = new p5(d, 'divergenceContainer');